...
bool
Block::typeCheck(uint32_t type) const{
	Block::element_const_iterator itr= std::find_if(m_subBlocks.begin(), m_subBlocks.end(),
            [type] (const Block& subBlock) { return subBlock.type() == type; });

	if(itr==m_subBlocks.end()){
		return false;
	}else{
		return true;
	}
...
/*
Block::element_iterator
Block::push_back_new_data(){
	resetWire();
	return m_subBlocks.push_back(element);
}
*/
Block::element_iterator
