 ...
 */
const time::milliseconds DEFAULT_INTEREST_LIFETIME = time::milliseconds(8000);
...
public: // Name and guiders
  ...
  bool
  hasHashFlag()  const{
//	  return HashFlag.hasWire();  //Nakazato
	  return true;  //Nakazato
  }
  ...
  const Interest&
  setHashFlagConst(int i);


/*  bool
  hasHashTTL() const{
	  return HashTTL.hasWire();
  }*/
  ...
  Interest&
  setHashTTL(uint32_t ttl);

  const Interest&
  setHashTTLConst(uint32_t ttl);

  std::size_t
  getConsumerID() const;

  Interest&
  setConsumerID(size_t id);
...
//  mutable Block HashFlag;
  mutable int HashFlag;
//  mutable Block HashTTL;
  mutable uint32_t HashTTL;

  mutable std::size_t consumerID;
};
