...
int
Interest::getHashFlag() const{
/*	if(!HashFlag.hasWire()){
		const_cast<Interest*>(this)->setHashFlag();
	}

	if(HashFlag.value_size()==sizeof(bool)){
		return *reinterpret_cast<const bool*>(HashFlag.value());
	}else{
		return readNonNegativeInteger(HashFlag);
	}*/
	return HashFlag;
}

uint32_t
Interest::getHashTTL() const{
/*	if(!HashTTL.hasWire()){
		const_cast<Interest*>(this)->setHashTTL(10);
	}

	if(HashTTL.value_size()==sizeof(uint32_t)){
		return *reinterpret_cast<const uint32_t*>(HashTTL.value());
	}else{
		return readNonNegativeInteger(HashTTL);
	}*/
	return HashTTL;
}

std::size_t
Interest::getConsumerID() const{
	return consumerID;
}
...
Interest&
Interest::setHashFlag(int i){
/*	bool flag=true;
	if(m_wire.hasWire() && HashFlag.value_size()==sizeof(bool)){
		std::memcpy(const_cast<uint8_t*>(HashFlag.value()),&flag,sizeof(flag));
	}else{
		HashFlag=makeBinaryBlock(tlv::HashFlag,
...
const Interest&
Interest::setHashFlagConst(int i){
/*	bool flag=true;
	if(m_wire.hasWire() && HashFlag.value_size()==sizeof(bool)){
			std::memcpy(const_cast<uint8_t*>(HashFlag.value()),&flag,sizeof(flag));
	}else{
...
Interest&
Interest::setHashTTL(uint32_t ttl){
/*	if(m_wire.hasWire() &&HashTTL.value_size()==sizeof(uint32_t)){
		std::memcpy(const_cast<uint8_t*>(HashTTL.value()),&ttl,sizeof(ttl));
	}else{
		HashTTL=makeBinaryBlock(tlv::HashTTL,
								reinterpret_cast<const uint8_t*>(&ttl),
								sizeof(ttl));
		m_wire.reset();
	}*/ //Nakazato
	HashTTL = ttl; //Nakazato
    m_wire.reset();
	return *this;
}
...
const Interest&
Interest::setHashTTLConst(uint32_t ttl){
/*	if(m_wire.hasWire() &&HashTTL.value_size()==sizeof(uint32_t)){
			std::memcpy(const_cast<uint8_t*>(HashTTL.value()),&ttl,sizeof(ttl));
		}else{
			HashTTL=makeBinaryBlock(tlv::HashTTL,
...
Interest&
Interest::setConsumerID(std::size_t id){
	consumerID=id;
	m_wire.reset();
	return *this;
}
...
Interest::wireEncode(EncodingImpl<TAG>& encoder) const
{
  ...
  // (reverse encoding)
  totalLength += prependNonNegativeIntegerBlock(encoder, tlv::consumerID,static_cast<uint64_t>(consumerID));
  totalLength += prependNonNegativeIntegerBlock(encoder, tlv::HashTTL, static_cast<uint64_t>(HashTTL));
  totalLength += prependNonNegativeIntegerBlock(encoder, tlv::HashFlag, static_cast<uint64_t>(HashFlag));
  ...
  /*
  getHashFlag();
  totalLength+=encoder.prependBlock(HashFlag);
  */

  /* //Nakazato
  getHashTTL();
  totalLength+=encoder.prependBlock(HashTTL);
  */
...
//  HashFlag=m_wire.get(tlv::HashFlag); // Nakazato

//  HashTTL=m_wire.get(tlv::HashTTL); // Nakazato
  ...
  HashFlag = static_cast<int>(readNonNegativeInteger(m_wire.get(tlv::HashFlag)));
  HashTTL =static_cast<uint32_t>(readNonNegativeInteger(m_wire.get(tlv::HashTTL)));
  consumerID=static_cast<std::size_t>(readNonNegativeInteger(m_wire.get(tlv::consumerID)));
}
...
operator<<(std::ostream& os, const Interest& interest)
{
  ...
  //if(interest.hasHashFlag()){
	  os << delim << "ndn.HashFlag=" << interest.getHashFlag();
	      delim = '&';
  //}

  //if(interest.hasHashTTL()){
	  os << delim << "ndn.HashTTL=" << interest.getHashFlag();
	      delim = '&';
  //}

	  os << delim <<"ndn.consumerID=" <<interest.getConsumerID();
  return os;
}
