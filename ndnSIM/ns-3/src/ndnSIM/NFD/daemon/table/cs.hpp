...
public:
  ...
  const bool
  isfindData(const Data& data) const;

  bool
  isEmpty() const;

  const Data&
  searchNearestConsumerIDData(const Interest& interest) const;

  bool
  checkForConsumerID(const Interest& interest) const;

  const Data&
  getNearConsumerIDData(const Interest& interest) const;


  shared_ptr<lp::IncomingFaceIdTag>
  searchNearestHashData(const Interest& interest) const;
/*
  void
  setOutFaceId(const Interest& interest, FaceId id);*/
