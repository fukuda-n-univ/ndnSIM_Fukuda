...
Face&
Strategy::lookupCs(const pit::Entry& pitEntry) const{
	const Cs& cs=m_forwarder.getCs();
	const FaceTable& faceTable=m_forwarder.getFaceTable();

	const Interest& interest=pitEntry.getInterest();

	shared_ptr<lp::IncomingFaceIdTag> DataTag=cs.searchNearestHashData(interest);

	if(DataTag!=nullptr){
		Face* inFace=faceTable.get(*DataTag);
		return *inFace;
	}
