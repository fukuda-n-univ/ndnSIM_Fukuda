...
/*
	...
	if(this->isCsEmpty()==true){
		...
		do{
		    ...
		    std::mt19937 mt;
			mt.seed(rnd());
			boost::random::uniform_int_distribution<> randomNumGenerator(0,size-1);

			counter=randomNumGenerator(mt);
			randomFace=faceTable.getNumFace(counter);

		}while(randomFace!=nullptr &&randomFace->getId()!=face::INVALID_FACEID);
