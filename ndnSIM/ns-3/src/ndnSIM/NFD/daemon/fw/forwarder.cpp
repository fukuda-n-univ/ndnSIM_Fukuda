...
Forwarder::onIncomingInterest(Face& inFace, const Interest& interest)
{
  ...
  NFD_LOG_DEBUG("onIncomingInterest face=" << inFace.getId() <<
				...
				"interest hashFlag=" << interest.getHashFlag() <<
				"interest consumerID=" << interest.getConsumerID() );
  ...
  if(interest.getHashFlag()>0){ // Nakazato

  		if(interest.getHashTTL()>0 && interest.getHashTTL()<=10){
  			NFD_LOG_DEBUG("TTL="<<interest.getHashTTL());
  			...
  			// detect duplicate Nonce with Dead Nonce List
  			  ...
  			  if (hasDuplicateNonceInDnl) {
  			    ...
  			    this->onHashFail(inFace, interest);
  			    return;
  			  }
  			...
  			uint32_t TTL=interest.getHashTTL();
  			 TTL--;
       	    Interest& i=const_cast<Interest&>(interest);
       	    i.setHashTTL(TTL);

       	    if(interest.getHashFlag()==2){
       	    	NFD_LOG_DEBUG("first pitEntry");
       	    	pitEntry->setHashStartFlag();
       	    	i.setHashFlag(1);
       	    }else if(interest.getHashFlag()==11){
       	    	pitEntry->setConsumerHashFlag();
       	    }
  			...
  			const Interest& interestHash=const_cast<const Interest&>(i);
  			 ...
  			 // detect duplicate Nonce in PIT entry
  			  bool hasDuplicateNonceInPit = fw::findDuplicateNonce(*pitEntry, interestHash.getNonce(), inFace) !=
  			                                fw::DUPLICATE_NONCE_NONE;
  			  if (hasDuplicateNonceInPit) {
  			    // goto Interest loop pipeline
  			    this->onHashFail(inFace, interestHash);
  			    return;
  			  }
  			  ...
  			  if (!isPending) {
  				 ...
  				 if(interest.getHashFlag()>10){
  					 //if(m_cs.isEmpty()==true || m_cs.checkForConsumerID(interestHash)==false){
  					 if(m_cs.isEmpty()==true){
  						 this->onContentStoreMiss(inFace,pitEntry,interestHash);
  						 return;

  					 }else{

  					/*	 if(m_cs.checkForConsumerID(interestHash)==false){
  							 this->onContentStoreMiss(inFace,pitEntry,interestHash);
  							 return;
  						 }*/
  						 const Data& datatmp=m_cs.searchNearestConsumerIDData(interestHash);

  						const Name& dataName=interestHash.getName();

  						auto emptydata = make_shared<Data>();

  						emptydata->setName(dataName);
  						emptydata->setFreshnessPeriod(::ndn::time::milliseconds(m_freshness.GetMilliSeconds()));
  						emptydata->setContent(make_shared< ::ndn::Buffer>(1024));

  						const Block& encodedData  =datatmp.wireEncode();
  						NFD_LOG_DEBUG("encoded data type ="<< encodedData.type());

  						Block& content=const_cast<Block&>(emptydata->getContent());
  						//NFD_LOG_DEBUG("content block size = "<<content.size());

  						content.push_back(encodedData);

  						//NFD_LOG_DEBUG("content block size = "<<content.size());
  						const Block& contentWithData=const_cast<const Block&>(content);

  						emptydata->setContent(contentWithData);

  						NFD_LOG_DEBUG("outside content type =" <<contentWithData.type());
  						NFD_LOG_DEBUG("typeCheck of subBlocks ="<< contentWithData.typeCheck(tlv::Data));
  						NFD_LOG_DEBUG("data name for forwarding="<<emptydata->getName() <<
  										"content size="<<contentWithData.size() <<
										"consumer ID=" << interestHash.getConsumerID());
  						 ...
  						 if(m_keyLocator.size() > 0){
  						  ...
  						  emptydata->setSignature(signature);

  						 const Data& data=const_cast<const Data&>(*emptydata);

  						 this->onContentStoreHit(inFace,pitEntry,interestHash,data);
  						 return;

  					 }
  				}else{
  					m_cs.find(interestHash,
  				...
  				/*}else if (m_csFromNdnSim == nullptr) {
  			      ...
  			      shared_ptr<Data> match = m_csFromNdnSim->Lookup(interestHash.shared_from_this());
  			      if (match != nullptr) {
  			        this->onContentStoreHit(inFace, pitEntry, interestHash, *match);
  			      }
  			      else {
  			        this->onContentStoreMiss(inFace, pitEntry, interestHash);
  			      }
...
void
Forwarder::onHashFail(Face& inFace,const Interest& interest){
	  ...
	  NFD_LOG_DEBUG("onHashFail face=" << inFace.getId() <<
	                " interest=" << interest.getName() <<
	                " send-Nack-HashFail");
...
{
  NFD_LOG_DEBUG("onContentStoreHit interest name=" << interest.getName());
  NFD_LOG_DEBUG("interest consumer ID= "<< interest.getConsumerID());
  NFD_LOG_DEBUG("Data name= "<< data.getName());
  NFD_LOG_DEBUG("inside content type=" << data.getContent().typeCheck(tlv::Data));
...
Forwarder::onOutgoingInterest(const shared_ptr<pit::Entry>& pitEntry, Face& outFace, const Interest& interest)
{
  ...
  /*int FaceIdInt=(int)outFace.getId();
  if(interest.getHashFlag()>10){
	  m_cs.setOutFaceId(interest,FaceIdInt);
  }*/
  // insert out-record
...
{
  // receive Data
  NFD_LOG_DEBUG("onIncomingData size="<<data.getContent().size());
  NFD_LOG_DEBUG("onIncomingData face=" << inFace.getId() << " data=" << data.getName());
  NFD_LOG_DEBUG("onIncomingData typecheck" << data.getContent().typeCheck(tlv::Data));
...
*/
  ...
  for (const shared_ptr<pit::Entry>& pitEntry : pitMatches) {
	...
	if(pitEntry->getConsumerHashFlag()==true){
		csSetDecision=true;
	}
  ...
  if(csSetDecision==true){
	  const Block& content=data.getContent();

	   // NFD_LOG_DEBUG("type ="<<content.type());
	  //  shared_ptr<Data> newData2 = make_shared<Data>(content);

	  NFD_LOG_DEBUG("element check="<<content.typeCheck(tlv::Data));
		if(content.typeCheck(tlv::Data)==true){

			const Block& block=content.get(tlv::Data);
			  shared_ptr<Data> newData = make_shared<Data>(block);
			  newData->removeTag<lp::HopCountTag>();

			  std::string nameUri=data.getName().toUri();
			  NFD_LOG_DEBUG( " name URI=" << nameUri);

			  if(nameUri.find("/localhost")==std::string::npos){
				  m_cs.insert(*newData);
			  }
			  // CS insert
			 	  /*if (m_csFromNdnSim == nullptr){
...
{
	NFD_LOG_DEBUG("onOutgoingData name= " << data.getName());
	NFD_LOG_DEBUG("inside content typecheck=" << data.getContent().typeCheck(tlv::Data));
...
Forwarder::onIncomingNack(Face& inFace, const lp::Nack& nack)
{
  ...
  if(pitEntry->getHashStartFlag()==true){
	  Interest& i=const_cast<Interest&>(nack.getInterest());
	  i.setHashFlag(0);
	  const Interest& interest=const_cast<const Interest&>(i);

	  pitEntry->resetHashStartFlag();
	  pitEntry->deleteOutRecord(inFace);

	  // has in-record?
	  pit::InRecordCollection::iterator inRecord = pitEntry->in_begin();
...
//	      [&] (fw::Strategy& strategy) { strategy.afterReceiveInterest(inFace, interest, pitEntry); });
	  this->dispatchToStrategy(*pitEntry,
	      [&] (fw::Strategy& strategy) { strategy.afterReceiveInterest(inRecord->getFace(), interest, pitEntry); });
	  return;
  }


//  if(nack.getInterest().getHashFlag()==0){
